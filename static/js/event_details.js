...
function editComment(button, commentId, commentText) {
  ...
  $(`#comment${commentId}`).html(`
    <form id="editComment${commentId}Form" method="post" action="edit_comment/${commentId}">
      <input type="hidden" name="csrfmiddlewaretoken" value="${csrftoken}">
      <label for="id_comment">Edit comment:</label>
      <textarea name="comment" class="form-control" required="" id="id_comment" style="height: 20vh;">${commentText}</textarea>
      <input class="btn edit-btn mt-3" type="submit" id="saveComment" value="Save">
    </form>
  `);
